...
| 启动时自动检查 / 更新频道 / 自动下载          | 常规     | 手动"检查更新"已接入(2026-07-14,GitHub Releases 源)。剩这三项**设置页开关**待做:启动后台静默检查、稳定/预览频道切换、自动下载;核心 `UpdateService` 已就绪,接开关即可              |
...
- ✅ 关于页"检查更新"已接入(2026-07-14):Velopack `UpdateService` → **GitHub Releases**(`GithubSource` + 按架构 channel `win-x64/arm64`,匹配 CI `vpk pack --channel`),流程 检查→下载(进度)→"重启并更新"按钮;非 Setup.exe 安装(便携/MSI)、已最新、失败三态均如实提示(8 个五语言文案)。`allowPreRelease=true`(beta 阶段;发正式版可置 false)。CI 已恢复上传 `releases.$rid.json`+`*-full.nupkg` 到 Release。"更新日志"仍禁用。
