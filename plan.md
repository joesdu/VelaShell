...
> 最近更新:**2026-07-14**(§16:自研 VelaDock 落地合并、主窗回退平台原生标题栏、终端行号/时间侧栏、本地终端 Job Object 秒杀进程树、Windows MSI 安装包与自定义安装目录、集中式包管理、Avalonia 12.1.0、全项目 XML 注释与每项目 README)。
...
- **窗口壳:平台原生标题栏(2026-07-13 定稿,用户反馈后从自绘回退)**:主窗改用操作系统**原生 chrome** —— 拖动 / 贴靠 / 最小化-最大化-关闭全部交还 OS(Win11 边缘贴靠与 Snap Layouts 原生手感,提交 `7580052`/`ce71b32`);曾经的 `WindowDecorations="None"` 全自绘窗口壳已弃用(回退原因见下「踩坑备忘」)。原生标题栏之下保留一条 36px 工具条:左 logo+产品名,右 全局功能图标组(搜索/复制/分屏/隧道/命令面板,经命令注册表;组同步/广播未实现禁用)。**文字菜单(会话/编辑/…)已整体移除**——与命令面板功能重复(用户决策);随之移除设置里的"显示菜单栏"开关(`ShowMenuBar` 存储字段保留兼容)。对话框仍为自绘无边框(`SystemDecorations=None`),仅主窗回退原生。
  - **踩坑备忘(自绘壳为何回退,Avalonia 12.0.5 观察)**:①`VisualRoot as Window` 恒为 null(视觉根是 TopLevelHost),取窗口必须走逻辑树 `FindLogicalAncestorOfType<Window>()`——曾令标题栏按钮/拖动看似"无输入"数小时;②`ExtendClientAreaToDecorationsHint`/`BorderOnly` 的托管装饰(`WindowDrawnDecorations`)会绘制重复标题与含"全屏"的按钮,且 `WindowDecorationsElementRole` 的输入重定向未落地(HT*BUTTON 点击无动作、User 角色不可点),BorderOnly 还丢 WS_CAPTION(HTCAPTION 拖动与最小/最大化动画失效,issue #21160/#21212)——整套 extend 机制在 12.0.5 不可用,故弃用。
...
**B. 主窗回退平台原生标题栏(用户反馈)**:详见 §6(已改写)。自绘无边框壳在 Avalonia 12.x 的拖动/贴靠保真不足,回退为**操作系统原生 chrome**,窗口按钮/移动/Snap Layouts 交还 OS;并补 `c12a8ff`(Avalonia 12 `VisualRoot` 非 `Window`,取窗口须走逻辑树)与 `ce71b32`(Win11 贴靠布局面板 + 最大化拖动原生手感)。对话框仍自绘无边框。
