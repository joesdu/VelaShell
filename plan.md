...
- **自绘窗口壳(2026-07-12 定稿,用户决策)**:主窗 `WindowDecorations="None"`(与全部对话框同款全自绘模式);`Views/TitleBarView` 自绘标题栏 —— 左 logo+产品名,右 全局功能图标组(搜索/复制/分屏/隧道/命令面板,经命令注册表;组同步/广播未实现禁用)+ 最小化/最大化/关闭;空白区 `BeginMoveDrag` 拖动(原生移动循环,Win11 边缘贴靠有效;最大化下先还原)、双击切最大化,最大化按钮图标随状态切换;窗口四周 5px 自绘缩放抓取区(`BeginResizeDrag`,最大化时隐藏);最大化占工作区不盖任务栏。**文字菜单(会话/编辑/…)已整体移除**——与命令面板功能重复(用户决策);随之移除设置里的"显示菜单栏"开关(`ShowMenuBar` 存储字段保留兼容)。
  - **踩坑备忘(Avalonia 12.0.5)**:①`VisualRoot as Window` 恒为 null(视觉根是 TopLevelHost),取窗口必须走逻辑树 `FindLogicalAncestorOfType<Window>()`——曾令标题栏按钮/拖动看似"无输入"数小时;②`ExtendClientAreaToDecorationsHint`/`BorderOnly` 的托管装饰(`WindowDrawnDecorations`)会绘制重复标题与含"全屏"的按钮,且 `WindowDecorationsElementRole` 的输入重定向未落地(HT*BUTTON 点击无动作、User 角色不可点),BorderOnly 还丢 WS_CAPTION(HTCAPTION 拖动与最小/最大化动画失效,issue #21160/#21212)——整套 extend 机制在 12.0.5 不可用,故弃用。
