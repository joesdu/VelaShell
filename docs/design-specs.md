...
>
> ⚠️ **实现差异说明**：本文档为设计稿原始提取。实现中主窗已改用**操作系统原生标题栏**（见下方「窗口结构」§2），
> 且下方「菜单栏」中的 6 个**文字菜单项（会话/编辑/操作/搜索/工具/帮助）已整体移除**（与命令面板功能重复，
> 产品决策）——仅保留右侧的功能图标组（搜索/复制/分屏/隧道/命令面板）。其余布局与 token 仍为 UI 重建依据。
...
## 窗口结构（§2 权威）
- **操作系统原生标题栏**（不自绘）→ 菜单栏 36 → (侧栏 260 ‖ 右区) → 状态栏 24。
- 右区：标签栏 36 → 终端(fill) → 文件区 220。
