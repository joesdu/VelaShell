...
| ⏳ | `plan.md` §118 | **窗口外框跨平台适配**:已开 [velashell-docs#70](https://github.com/VelaShellLabs/velashell-docs/pull/70)(中英各 3 个文件),**与宿主 PR 互相引用,一起合**。内容:`{zh,en}/host/architecture.md` §5「窗口壳」的 ⚠️ 限定为 Win32、新增「各平台的外框」(对照表、`WindowChrome` 与 XAML 约定、Avalonia 12.1.3 源码依据、验收状态);`{zh,en}/host/交互与界面规格.md` §2 补 macOS 红绿灯与各平台的弹窗 / 独立窗口外框、「窗口标题栏说明」补 macOS 一句、阴影一条补 Wayland 阴影宽度;`{zh,en}/host/design-specs.md` 实现差异说明补 macOS 红绿灯;全部窗口标题栏统一 28 的口径也已写进这三篇(36px / 48px 的旧说法已改),设置窗口与消息框保持 48 的例外、§4A.2 X Server 按钮排最后也已写进去。实机验收做完后改掉 architecture 里「验收」那一段 |
