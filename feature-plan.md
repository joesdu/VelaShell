...
真正的空档集中在三处：**① 终端本身的现代化**（OSC 8 / OSC 133 / 键盘复制模式）、
...
| :---: | :---: | --- | --- | --- |
| ⏳ | 🟠 P1 | **OSC 133 提示符语义 + 命令块** | Warp / WezTerm / iTerm2 | **本轮性价比最高的一项。** 三块料都已经在手上：① `TerminalEmulator` 已经解析 OSC 7（`ParseOsc7Path`），加 133 是同一条分发路径；② bash 会话**已经在注入 `PROMPT_COMMAND` 钩子**（§18-F），加一段 `OSC 133;A/B/C/D` 即可，`RemoteShellProbe` 那道「对端认不认 sh 语法」的闸也现成；③ `GutterFoldModel` 已经有折叠模型，命令块的折叠/跳转正好挂上去。做成之后能一次解锁：跳到上/下一条提示符、选中「这条命令的输出」、一键重跑、失败命令标红 —— 这些是 Warp 的核心卖点，而**对标矩阵里六家没有一家有** |
| ⏳ | 🟠 P1 | **OSC 8 超链接** | Tabby / 现代 CLI（`ls --hyperlink`、`gh`、`delta`） | 现在只有**正则识别**的 URL（`SemanticMatcher` + `VelaTerminalControl:3511` 的 `LaunchUriAsync`）。OSC 8 的区别是**显示文本与目标 URI 可以不同**，正则永远认不出来。落点：`CellFlags` 旁边加一个 link-id、OSC 分发里存 URI 表、命中时复用已有的点击路径。⚠️ 每格多一个引用会动到渲染热路径，`Terminal.RenderTests` 的像素回归正好守着 |
...
1. **OSC 133 命令块**（A 组）—— 料都在手上（OSC 7 解析、`PROMPT_COMMAND` 注入、折叠模型），
   而对标的六家**一家都没有**。这是能拉开差距的一项，不是追平的一项。
