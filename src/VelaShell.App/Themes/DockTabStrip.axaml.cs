...
namespace VelaShell.App.Themes;

/// <summary>DockTabStrip.axaml 的代码后端:标签列表下拉(设计 nunbT tabListDrop)。
/// 菜单在点击时按当前打开的标签动态生成,选中即激活对应标签。</summary>
public partial class DockTabStripResources : ResourceDictionary
{
    ...
    private void TabListDrop_Click(object? sender, RoutedEventArgs e)
    {
        if (sender is not Button button
            || button.DataContext is not IDocumentDock dock
            || dock.VisibleDockables is not { Count: > 0 } dockables)
        {
            return;
        }
