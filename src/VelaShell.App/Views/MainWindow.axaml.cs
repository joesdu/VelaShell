...
public partial class MainWindow : Window
{
    ...
    private VelaShell.Core.Data.ISettingsService? _settingsService;
    ...
    private async void OnWindowOpened(object? sender, EventArgs e)
    {
        if (DataContext is MainWindowViewModel vm)
        {
            ...
            if (vm.Sidebar.SessionTree is { } tree)
            {
                ...
                tree.OpenSftpRequested += profile => Avalonia.Threading.Dispatcher.UIThread.Post(async () =>
                {
                    ...
                    if (tab is { ConnectionStatus: VelaShell.Core.Models.SessionStatus.Connected }
        ...
        if (App.Current is App { Services: { } services }
            && services.GetService<VelaShell.Core.Data.ISettingsService>() is { } settingsService)
        {
    ...
    private async Task RestoreSessionsAsync(AppSettings? settings)
    {
        if (settings?.General.RestoreSessionsOnStartup != true
            ...
            || services.GetService<VelaShell.Core.Data.ISessionRepository>() is not { } repository)
        {
            return;
        }
    ...
    {
        var confirmed = await MessageDialog.ConfirmAsync(this, "关闭 VelaShell",
    ...
    private async Task OpenProfileDialogAsync(SessionProfile? existing)
    {
        ...
        var connectionProfileViewModel = new ConnectionProfileViewModel(
            ...
            sessionRepository: app.Services.GetService<VelaShell.Core.Data.ISessionRepository>(),
    ...
    private async Task<SessionProfile?> PromptCredentialsAsync(SessionProfile profile)
    {
        ...
        if (App.Current is App app
            && app.Services?.GetService<VelaShell.Core.Ssh.IHostKeyService>() is { } hostKeys)
        {
            try
            {
        ...
        if (result.AuthMethod == VelaShell.Core.Models.AuthMethod.Password)
        {
            // 交接点:SecureString → 管线所需的明文,随即释放 SecureString。
            using (result.Password)
            {
                profile.Password = VelaShell.App.Security.SecureStringConvert.ToPlaintext(result.Password);
            }
