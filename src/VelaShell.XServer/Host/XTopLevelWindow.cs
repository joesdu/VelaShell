...
public sealed class XTopLevelWindow
{
    ...
    internal XTopLevelWindow(uint id, PixelGate pixelGate, Func<(uint[] Pixels, int Width, int Height)?> pixels)
    {
        Id = id;
    ...
    /// <summary>
    /// 拷贝当前像素(深度 24 的窗口是 <c>0x00RRGGBB</c>,高 8 位无意义;<see cref="HasAlpha" /> 时是预乘的 <c>0xAARRGGBB</c>),
    ...
    /// 宽高取自此刻的缓冲本身 —— 宿主之前读到的 <see cref="Width" /> / <see cref="Height" /> 可能已经过时(客户端刚改了尺寸)。
    /// </summary>
    /// <remarks>
    /// <para>
    /// <paramref name="reader" /> 执行期间服务端不画(执行线程在等锁),所以要快:只拷要的那几块,不要在里面分配大块内存、
    /// 等别的锁或同步调服务端的方法。宿主在等这把锁时,执行线程会尽快让出(执行完手上那一项就放锁,读完再拿)。
    /// </para>
    /// </remarks>
