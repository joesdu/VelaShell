...
/// 其余键(方向、功能、修饰)不变。键码范围、固定键与列序见 <see cref="HostKeymap" />。
/// </summary>
/// <remarks>
/// <para>
...
internal static partial class WindowsKeymap
{
    ...
    private static (uint L1, uint L2, uint L3, uint L4) Translate(byte keycode, nint layout)
    {
        ...
        return (KeysymFor(vk, scancode, shift: false, altGr: false, layout), KeysymFor(vk, scancode, shift: true, altGr: false, layout),
            KeysymFor(vk, scancode, shift: false, altGr: true, layout), KeysymFor(vk, scancode, shift: true, altGr: true, layout));
    }
    ...
    private static unsafe uint KeysymFor(uint vk, uint scancode, bool shift, bool altGr, nint layout)
    {
        ...
        return count < 0 ? HostKeymap.DeadKeysym(buffer[0]) : HostKeymap.Keysym(buffer[0]);
    }
