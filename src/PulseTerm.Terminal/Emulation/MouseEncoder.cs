...
public static class MouseEncoder
{
        ...
        TerminalModes modes)
    {
        ...
        {
            MouseEncoding.Sgr => Ascii(
                $"\x1b[<{cb};{cx};{cy}{(type == TerminalMouseEventType.Release ? 'm' : 'M')}"),
            MouseEncoding.Urxvt => Ascii($"\x1b[{cb + 32};{cx};{cy}M"),
    ...
    private static byte[] EncodeX10(int cb, int cx, int cy)
    {
        ...
        return new byte[] { 0x1b, (byte)'[', (byte)'M', b, x, y };
    }
