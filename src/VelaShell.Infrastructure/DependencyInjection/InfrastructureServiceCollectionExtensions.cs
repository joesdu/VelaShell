...
public static class InfrastructureServiceCollectionExtensions
{
    public static IServiceCollection AddVelaShellInfrastructure(this IServiceCollection services)
    {
        ...
        services.AddSingleton<SonnetDbEngine>(serviceProvider =>
            new SonnetDbEngine(serviceProvider.GetRequiredService<VelaShellStoragePaths>()));
        ...
        {
            var paths = serviceProvider.GetRequiredService<VelaShellStoragePaths>();
        ...
        services.AddSingleton<VelaShell.Core.Ssh.IHostKeyService>(serviceProvider =>
        {
            var paths = serviceProvider.GetRequiredService<VelaShellStoragePaths>();
        ...
        services.AddSingleton<VelaShell.Core.Ssh.ISshKeyService>(_ => new SshKeyService());

        services.AddSingleton<VelaShell.Core.Ssh.ISecurityAlertService>(serviceProvider =>
            new VelaShell.Core.Ssh.SecurityAlertService(
                serviceProvider.GetRequiredService<ISettingsService>(),
                serviceProvider.GetService<IAuditLogService>()));

        services.AddSingleton<ISshConnectionService>(serviceProvider =>
        {
            var hostKeyService = serviceProvider.GetRequiredService<VelaShell.Core.Ssh.IHostKeyService>();
            var settingsService = serviceProvider.GetRequiredService<ISettingsService>();
            var hostKeyPrompt = serviceProvider.GetService<VelaShell.Core.Ssh.IHostKeyPrompt>();
            var securityAlerts = serviceProvider.GetService<VelaShell.Core.Ssh.ISecurityAlertService>();
        ...
        services.AddSingleton<VelaShell.Core.Services.ISessionMetricsService>(sp =>
    ...
    private static VelaShell.Core.Models.SecurityOptions GetSecurityOptions(ISettingsService? settingsService)
    {
        try
        {
            return settingsService?.GetSettingsAsync().GetAwaiter().GetResult().Security
                ?? new VelaShell.Core.Models.SecurityOptions();
        }
        catch
        {
            return new VelaShell.Core.Models.SecurityOptions();
        }
    }
    ...
    private static ISshClientWrapper CreateSshClientWrapper(
        ...
        VelaShell.Core.Ssh.ISecurityAlertService? securityAlerts = null)
    {
    ...
    private static SshClient BuildSshClient(
        ...
        VelaShell.Core.Ssh.IHostKeyPrompt? hostKeyPrompt,
        VelaShell.Core.Ssh.ISecurityAlertService? securityAlerts)
    {
        ...
        if (hostKeyService is not null)
        {
            ...
            client.HostKeyReceived += (_, e) =>
            {
                ...
                switch (verification)
                {
                    case VelaShell.Core.Ssh.HostKeyVerification.Trusted:
                        e.CanTrust = true;
                        break;

                    case VelaShell.Core.Ssh.HostKeyVerification.Unknown:
                        bool trustNew = true;
