...
internal sealed class TmdsSshPortForwardHandle : IPortForwardHandle
{
    ...
    private TmdsSshPortForwardHandle(IDisposable forwardHandle, Action throwIfStopped, CancellationToken stopped)
    {
        ...
        SshClient client, PortForwardRequest request, CancellationToken cancellationToken)
    {
        switch (request.Kind)
        {
            case PortForwardKind.Local:
                {
                    LocalForward forward = await client.StartForwardAsync(
                        new IPEndPoint(IPAddressParse(request.BoundHost), (int)request.BoundPort),
                        new RemoteHostEndPoint(request.TargetHost!, (int)request.TargetPort!),
                        cancellationToken).ConfigureAwait(false);
                    return new TmdsSshPortForwardHandle(forward, forward.ThrowIfStopped, forward.Stopped);
                }
            case PortForwardKind.Remote:
                {
                    RemoteForward forward = await client.StartRemoteForwardAsync(
                        ...
                        new IPEndPoint(IPAddressParse(request.TargetHost!), (int)request.TargetPort!),
                        cancellationToken).ConfigureAwait(false);
                    return new TmdsSshPortForwardHandle(forward, forward.ThrowIfStopped, forward.Stopped);
                }
            case PortForwardKind.Dynamic:
                {
                    SocksForward forward = await client.StartSocksForwardAsync(
                        new IPEndPoint(IPAddressParse(request.BoundHost), (int)request.BoundPort),
                        cancellationToken).ConfigureAwait(false);
                    return new TmdsSshPortForwardHandle(forward, forward.ThrowIfStopped, forward.Stopped);
                }
    ...
    private static IPAddress IPAddressParse(string host)
    {
        return host is "0.0.0.0" or "*" ? IPAddress.Any : host == "::"
            ? IPAddress.IPv6Any : host is "localhost" or "127.0.0.1"
