...
public static class SshCompressorFactory
{
    ...
    /// <summary>这个算法名需不需要真的压缩。</summary>
    public static bool IsCompression(string algorithm) =>
        algorithm is SshAlgorithmNames.Zlib or SshAlgorithmNames.ZlibOpenSsh;

    /// <summary>造一个压缩器；<c>none</c> 返回直通的那个。</summary>
    public static ISshCompressor Create(string algorithm, int level = 6) =>
        IsCompression(algorithm) ? new ZlibCompressor(level) : NoCompression.Instance;
}
