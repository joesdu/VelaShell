...
2. **两阶段隔离**：先写行为规格（纯自然语言 + 报文字段表 + 时序图，零代码片段），
   再照规格实现；两个阶段不共享上下文。
3. **相似度门禁**：CI 每次提交都对上述项目做 token 级 n-gram 指纹比对
   （见 [`scripts/ssh/similarity-gate/`](../../scripts/ssh/similarity-gate/)），超阈值即构建失败。
   历史记录公开可查。

**2026-09-21 实测**（语料：Tmds.Ssh 197 个 `.cs` + SSH.NET 330 个 `.cs`）：

| 比对 | token 重合度 |
| --- | :-: |
| **本库 `src/` 对上述两者** | **0.16%** |
| SSH.NET 对 Tmds.Ssh（两个彼此独立的实现） | 2.41% |

第二行是**底噪**：两个互不相干的 .NET SSH 库，光是「都在直译同一批 RFC」
就会重合 2.41%。本库是 0.16% —— **比独立实现之间的底噪还低一个数量级。**

同一个检测器拿 Tmds.Ssh 比对它自己会报出 99.81% 覆盖、174 个文件超限并拒绝构建；
这一步在 CI 里每次都先跑，**它红不了就说明门禁已经失效**，上面的数字也就不必看了。

> 需要说明的是：协议常量（消息号、算法名字符串、wire 布局、错误码）在所有正确的实现里
> **必然相同** —— 它们是协议规定的事实，表达方式唯一。相似度门禁对这部分设有白名单。
