...
//
// 这个文件放 SFTP 的数值常量：有协议规定的（版本号、句柄长度上限、文件类型位），也有本库自己选的
// （块大小、报文上限、默认权限）—— 每一条的注释都写明是哪一种。
// 协议规定的码表（报文类型、状态码、pflags、ATTRS 位）各在自己的文件里，扩展名字符串在 SftpExtensionNames.cs。

namespace VelaShell.Ssh.Sftp;

/// <summary>SFTP 协议层的常量。</summary>
internal static class SftpProtocol
{
