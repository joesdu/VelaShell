...
public sealed class SshDataWriterTests
{
    ...
    public void UInt32_是大端序()
    {
        Assert.AreSequenceEqual(
            new byte[] { 0x12, 0x34, 0x56, 0x78 }, Write(w => w.WriteUInt32(0x12345678)));
    }
    ...
    {
        Assert.AreSequenceEqual(
            new byte[] { 0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF }, Write(w => w.WriteUInt64(0x0123456789ABCDEF)));
    }
    ...
    public void 布尔的真必须发1而不是别的非零值()
    {
        ...
        Assert.AreSequenceEqual(new byte[] { 1 }, Write(w => w.WriteBoolean(true)));
        Assert.AreSequenceEqual(new byte[] { 0 }, Write(w => w.WriteBoolean(false)));
    }
    ...
    {
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 3, (byte)'a', (byte)'b', (byte)'c' }, Write(w => w.WriteString("abc"u8)));
    }
    ...
    public void String_是字节串_可以含零字节()
    {
        ...
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 3, 0x61, 0x00, 0x62 }, Write(w => w.WriteString([0x61, 0x00, 0x62])));
    }
    ...
    public void Utf8String_按字节数而不是字符数给长度()
    {
        ...
        Assert.AreSequenceEqual(new byte[] { 0, 0, 0, 3, 0xE4, 0xB8, 0xAD }, actual);
    }
    ...
    public void Mpint_零编码为长度零的串_不是一个零字节()
    {
        ...
        Assert.AreSequenceEqual(new byte[] { 0, 0, 0, 0 }, Write(w => w.WriteMpint([])));
        Assert.AreSequenceEqual(new byte[] { 0, 0, 0, 0 }, Write(w => w.WriteMpint([0x00])));
        Assert.AreSequenceEqual(new byte[] { 0, 0, 0, 0 }, Write(w => w.WriteMpint([0x00, 0x00, 0x00])));
    }
    ...
    public void Mpint_最高位为一时补零字节()
    {
        ...
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 2, 0x00, 0x80 }, Write(w => w.WriteMpint([0x80])));

        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 3, 0x00, 0xFF, 0x01 }, Write(w => w.WriteMpint([0xFF, 0x01])));
    }
    ...
    {
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 1, 0x7F }, Write(w => w.WriteMpint([0x7F])));
    }
    ...
    public void Mpint_去掉前导零之后再判断是否要补()
    {
        ...
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 2, 0x00, 0x80 }, Write(w => w.WriteMpint([0x00, 0x00, 0x80])));

        // 0x00 0x7F → 去前导零得 0x7F → 不补。
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 1, 0x7F }, Write(w => w.WriteMpint([0x00, 0x7F])));
    }
    ...
    public void Mpint_RFC_4251_示例表()
    {
        ...
        {
            Assert.AreSequenceEqual(expected, Write(w => w.WriteMpint(value)), $"mpint({Convert.ToHexString(value)}) 编码错误");
        }
    }
    ...
    public void NameList_逗号分隔()
    {
        ...
        Assert.AreSequenceEqual(
            new byte[] { 0, 0, 0, 9, (byte)'z', (byte)'l', (byte)'i', (byte)'b', (byte)',',
                         (byte)'n', (byte)'o', (byte)'n', (byte)'e' }, actual);
    }
