...
/// <summary>
/// <see cref="OpenSshCertificatePaths.InferPrivateKeyPath" />:由证书路径反推配对私钥的那条 OpenSSH 命名约定。
/// </summary>
/// <remarks>
...
[TestClass]
[TestCategory("Ssh")]
public sealed class OpenSshCertificatePathsTests
{
    ...
    public void InferPrivateKeyPath_OnConventionalPair_ReturnsKeyPath()
    {
        ...
        Assert.AreEqual(key, OpenSshCertificatePaths.InferPrivateKeyPath($"{key}-cert.pub", Only(key)));
    }
    ...
    public void InferPrivateKeyPath_WhenKeyFileAbsent_ReturnsNull() =>
        ...
        Assert.IsNull(OpenSshCertificatePaths.InferPrivateKeyPath("/home/user/.ssh/id_ed25519-cert.pub", _ => false));

    [TestMethod]
    public void InferPrivateKeyPath_OnCustomName_ReturnsNull()
    {
        // 不按 ssh-keygen 的后缀命名就推不出来:去掉 ".pub" 得到的不是私钥,而是公钥。
        Assert.IsNull(OpenSshCertificatePaths.InferPrivateKeyPath("/home/user/.ssh/id_ed25519.pub", _ => true));
        Assert.IsNull(OpenSshCertificatePaths.InferPrivateKeyPath("/home/user/.ssh/prod-cert", _ => true));
    }

    [TestMethod]
    public void InferPrivateKeyPath_OnBareSuffix_ReturnsNull() =>
        // 文件名整个就是后缀:剥掉之后剩空串,那不是路径。
        Assert.IsNull(OpenSshCertificatePaths.InferPrivateKeyPath("-cert.pub", _ => true));

    [TestMethod]
    public void InferPrivateKeyPath_OnEmptyInput_ReturnsNull()
    {
        Assert.IsNull(OpenSshCertificatePaths.InferPrivateKeyPath(null, _ => true));
        Assert.IsNull(OpenSshCertificatePaths.InferPrivateKeyPath("   ", _ => true));
    }
